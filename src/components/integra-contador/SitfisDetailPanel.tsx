    ...
    <SheetContent side="right" className="w-full p-0 sm:max-w-4xl">
      {client && <div className="flex h-full flex-col">
        <SheetHeader className="border-b p-5 pr-12">
          ...
          <SheetDescription>{client.document || 'CNPJ não informado'} · Consulta {client.consulted_at ? new Date(client.consulted_at).toLocaleString('pt-BR') : 'não realizada'}</SheetDescription>
        </SheetHeader>
        <ScrollArea className="flex-1">
          <div className="space-y-5 p-5">
            {!parsed ? <p className="rounded-sm border bg-muted/30 p-4 text-sm text-muted-foreground">Não foi possível extrair informações estruturadas com segurança. Consulte o relatório original.</p> : <>
              <section><h3 className="mb-2 font-semibold">Declarações omitidas <Badge variant="secondary" className="ml-2">{parsed.omissions.length}</Badge></h3>{parsed.omissions.length ? parsed.omissions.map((item, i) => <div key={i} className="mb-2 rounded-sm border p-3 text-sm"><div className="font-medium">{item.declaration}</div><p className="my-1 text-muted-foreground">{item.reason}</p><div className="my-2 flex flex-wrap gap-1">{item.competencies.length ? item.competencies.map(c => <Badge key={c} variant="outline" className="rounded-sm">{c.slice(5)}/{c.slice(0,4)}</Badge>) : <span className="text-muted-foreground">Informação não disponível</span>}</div><Trace source={item.source} /></div>) : <p className="text-sm text-muted-foreground">Nenhuma omissão identificada.</p>}</section>
