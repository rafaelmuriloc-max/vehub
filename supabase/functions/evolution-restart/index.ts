...
Deno.serve(async (req) => {
  ...
  try {
    const res = await fetch(`${url}/instance/restart/${instance}`, {
      method: "POST",
      headers: { apikey: key },
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
      return new Response(JSON.stringify({ ok: false, error: `Evolution API ${res.status}`, details: data, transient: res.status >= 500 }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    return new Response(JSON.stringify({ ok: true, raw: data }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (err) {
    return new Response(JSON.stringify({ ok: false, error: String(err), transient: true }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  }
});