...
AS $$
  ...
  WHERE
    CASE p_tab
      ...
      WHEN 'in_progress' THEN c.status = 'open' AND (c.assigned_to IS DISTINCT FROM p_user)
      ELSE TRUE
    END
  ORDER BY COALESCE(lm.created_at, c.updated_at) DESC;
$$;

GRANT EXECUTE ON FUNCTION public.get_chat_inbox(uuid, text) TO authenticated;