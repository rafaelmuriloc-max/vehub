...
BEGIN
  ...
  LOOP
    -- Extract template name: "[Template: NAME]"
    tpl_name := substring(msg_row.content from 13 for length(msg_row.content) - 13);

    -- Find closest log within ±90s with same template_name
    SELECT * INTO log_row
      FROM whatsapp_logs wl
     WHERE wl.template_name = tpl_name
       AND wl.sent_at BETWEEN msg_row.created_at - interval '90 seconds'
                          AND msg_row.created_at + interval '90 seconds'
